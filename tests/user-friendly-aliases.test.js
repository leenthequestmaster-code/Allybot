...
test('user-friendly aliases remain attached to canonical commands', () => {
  ...
  const collaboration = collectCommands(createCollaborationPlugin(whatsapp), {
    collaboration: { startReminderDispatcher() {} },
  })
  const knowledge = collectCommands(createKnowledgePlugin(whatsapp))
  const canon = collectCommands(createCanonPlugin(whatsapp))
  ...
  const personalization = collectCommands(createPersonalizationPlugin(whatsapp))
  ...
  assertAlias(canon, 'setcanon', 'cerita')
  ...
  assertAlias(personalization, 'myprefs', 'prefs')
  assertAlias(personalization, 'verbosity', 'balasan')
