...
test('Codebase generator redacts numeric WhatsApp identifiers in snapshots', async () => {
  const root = await createFixture()
  try {
    await writeFile(join(root, 'src', 'identity.ts'), "export const group = '120363426754036308@g.us'\n")
    await runGenerator(root)
    const snapshot = await readFile(join(root, 'Codebase', 'snapshot', 'src', 'identity.ts'), 'utf8')
    assert.equal(snapshot.includes('120363426754036308@g.us'), false)
    assert.match(snapshot, /<jid-redacted@g\.us>/)
  } finally {
    await rm(root, { recursive: true, force: true })
  }
})
