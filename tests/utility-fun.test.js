...
test('random replies with an in-range number and rejects malformed ranges', async () => {
  const harness = await createHarness()
  try {
    const [first] = await harness.send('rand-1', nextSender(), '120363200000000101@g.us', '!random 1 3')
    assert.match(first.text, /Angka acaknya: \*[1-3]\*/)

    const seen = new Set()
    for (let index = 0; index < 40; index += 1) {
      const [reply] = await harness.send(`rand-loop-${index}`, nextSender(), '120363200000000102@g.us', '!random 1 3')
      seen.add(Number(/\*([1-3])\*/.exec(reply.text)[1]))
    }
    assert.equal(seen.size >= 2, true, `expected multiple outcomes across 40 draws, saw ${[...seen]}`)

    const [missingMax] = await harness.send('rand-2', nextSender(), '120363200000000103@g.us', '!random 5')
    assert.match(missingMax.text, /Format: !random <min> <max>/)
    assert.match(missingMax.text, /Contoh: `!random 1 100`/)

    const [inverted] = await harness.send('rand-3', nextSender(), '120363200000000104@g.us', '!random 10 2')
    assert.match(inverted.text, /Format: !random <min> <max>/)

    const [nonInteger] = await harness.send('rand-4', nextSender(), '120363200000000105@g.us', '!random 1.5 3')
    assert.match(nonInteger.text, /Format: !random <min> <max>/)

    const [outOfBounds] = await harness.send('rand-5', nextSender(), '120363200000000106@g.us', '!random -1000000 1000001')
...
test('choose picks one pipe-separated option and enforces arity and length bounds', async () => {
  ...
  try {
    const seen = new Set()
    for (let index = 0; index < 40; index += 1) {
      const [reply] = await harness.send(`choose-${index}`, nextSender(), '120363200000000107@g.us', '!choose teh | kopi | susu')
      seen.add(/Pilihanku: \*(teh|kopi|susu)\*/.exec(reply.text)[1])
    }
    assert.equal(seen.size >= 2, true, `expected multiple outcomes across 40 draws, saw ${[...seen]}`)

    const [oneOption] = await harness.send('choose-1', nextSender(), '120363200000000108@g.us', '!choose teh')
    assert.match(oneOption.text, /Format: !choose <opsi 1> \| <opsi 2>/)

    const many = Array.from({ length: 21 }, (_, index) => `opsi${index}`).join(' | ')
    const [tooMany] = await harness.send('choose-2', nextSender(), '120363200000000109@g.us', `!choose ${many}`)
    assert.match(tooMany.text, /Format: !choose/)

    const [longOption] = await harness.send('choose-3', nextSender(), '120363200000000110@g.us', `!choose ${'x'.repeat(81)} | teh`)
    assert.match(longOption.text, /Format: !choose/)

    const [trimmed] = await harness.send('choose-4', nextSender(), '120363200000000111@g.us', '!choose   teh   |   kopi  ')
...
test('flip always lands on Kepala or Ekor', async () => {
  ...
  try {
    ...
    for (let index = 0; index < 20; index += 1) {
      const [reply] = await harness.send(`flip-${index}`, nextSender(), '120363200000000112@g.us', '!flip')
...
test('roll reports per-die values with a consistent total and rejects malformed dice', async () => {
  ...
  try {
    for (let index = 0; index < 10; index += 1) {
      const [reply] = await harness.send(`roll-${index}`, nextSender(), '120363200000000113@g.us', '!roll 3d6')
    ...
    const [alias] = await harness.send('roll-alias', nextSender(), '120363200000000114@g.us', '!dice 2d20')
    assert.match(alias.text, /2d20: /)

    for (const bad of ['!roll 0d6', '!roll 11d6', '!roll 2d1', '!roll 2d101', '!roll abc', '!roll']) {
      const [reply] = await harness.send(`roll-bad-${bad.replace(/\W+/g, '-')}`, nextSender(), '120363200000000115@g.us', bad)
...
test('truth and dare always answer from the curated prompt pools, including aliases', async () => {
  ...
  try {
    ...
    for (let index = 0; index < 30; index += 1) {
      const [reply] = await harness.send(`truth-${index}`, nextSender(), '120363200000000116@g.us', '!truth')
      seenTruth.add(/🗣️ \*Truth:\* (.+)$/.exec(reply.text)[1])
    }
    assert.deepEqual([...seenTruth].every((prompt) => truthPool.includes(prompt)), true)

    const [aliasTruth] = await harness.send('truth-alias', nextSender(), '120363200000000117@g.us', '!jujur')
    ...
    for (let index = 0; index < 30; index += 1) {
      const [reply] = await harness.send(`dare-${index}`, nextSender(), '120363200000000118@g.us', '!dare')
      seenDare.add(/🎯 \*Dare:\* (.+)$/.exec(reply.text)[1])
    }
    assert.deepEqual([...seenDare].every((prompt) => darePool.includes(prompt)), true)

    const [aliasDare] = await harness.send('dare-alias', nextSender(), '120363200000000119@g.us', '!tantangan')
...
test('8ball echoes the question and answers from the curated pool; empty input shows usage', async () => {
  ...
  try {
    ...
    const seen = new Set()
    for (let index = 0; index < 40; index += 1) {
      const [reply] = await harness.send(`ball-${index}`, nextSender(), '120363200000000120@g.us', '!8ball apakah hari ini cerah?')
    ...
    const [empty] = await harness.send('ball-empty', nextSender(), '120363200000000121@g.us', '!8ball   ')
...
test('rps without a subcommand explains the PvP flow, including the help alias', async () => {
  const harness = await createHarness()
  try {
    const [help] = await harness.send('rps-help', nextSender(), '120363200000000122@g.us', '!rps')
    ...
    const [explicit] = await harness.send('rps-help-2', nextSender(), '120363200000000123@g.us', '!rps help')
...
test('rps challenge requires a group, a mentioned opponent, and no self-challenge', async () => {
  const harness = await createHarness()
  try {
    const groupJid = '120363200000000124@g.us'

    const [privateAttempt] = await harness.send('rps-pm', '6281200000001@s.whatsapp.net', '6281200000001@s.whatsapp.net', '!rps challenge @bob', ['6281200000002@s.whatsapp.net'])
    assert.match(privateAttempt.text, /Command challenge hanya bisa digunakan di grup/)

    const [noTarget] = await harness.send('rps-notarget', '6281200000010@s.whatsapp.net', groupJid, '!rps challenge')
    assert.match(noTarget.text, /Mention atau reply pemain yang ingin ditantang/)

    const self = '6281200000020@s.whatsapp.net'
...
test('rps challenge announces in the group and DMs both players', async () => {
  const harness = await createHarness()
  try {
    const groupJid = '120363200000000125@g.us'
    const alice = '6281200000003@s.whatsapp.net'
    const bob = '6281200000004@s.whatsapp.net'
    ...
    assert.match(announcements[1].text, /Kamu menantang @6281200000004/)
    assert.match(announcements[1].text, /!rps batu/)

    assert.equal(announcements[2].remoteJid, bob)
    assert.match(announcements[2].text, /@6281200000003 menantangmu untuk Suit PvP/)
...
test('rps choices are PM-only and need an active challenge; unknown choices show usage', async () => {
  const harness = await createHarness()
  try {
    const [groupChoice] = await harness.send('rps-group-choice', '6281200000005@s.whatsapp.net', '120363200000000126@g.us', '!rps batu')
    assert.match(groupChoice.text, /Private Chat/)

    const loner = '6281200000006@s.whatsapp.net'
    ...
    const [badChoice] = await harness.send('rps-bad', '6281200000007@s.whatsapp.net', '6281200000007@s.whatsapp.net', '!rps laser')
...
test('rps duel completes when both players pick in PM and announces the result everywhere', async () => {
  const harness = await createHarness()
  try {
    const groupJid = '120363200000000127@g.us'
    const alice = '6281200000011@s.whatsapp.net'
    const bob = '6281200000012@s.whatsapp.net'
...
test('rps status lists active challenges with both participants and pending choices', async () => {
  const harness = await createHarness()
  try {
    const groupJid = '120363200000000128@g.us'
    const alice = '6281200000021@s.whatsapp.net'
    const bob = '6281200000022@s.whatsapp.net'
    ...
    assert.match(status.text, /@6281200000021 vs @6281200000022/)
...
test('rps cancel drops the caller\u0027s challenge and frees both players', async () => {
  const harness = await createHarness()
  try {
    const groupJid = '120363200000000129@g.us'
    const alice = '6281200000031@s.whatsapp.net'
    const bob = '6281200000032@s.whatsapp.net'

    const [nothing] = await harness.send('cancel-empty', nextSender(), '120363200000000130@g.us', '!rps cancel')
