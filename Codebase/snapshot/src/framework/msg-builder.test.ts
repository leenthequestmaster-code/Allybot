...
`
  ...
  it('builds standard text payload when no rich or buttons specified', () => {
    const built = MsgBuilder.to('<jid-redacted@s.whatsapp.net>')
...
`
  ...
  it('generates correct nativeFlow additionalNodes for group and private chats', () => {
    const privateNodes = nativeFlowAdditionalNodes('<jid-redacted@s.whatsapp.net>')
    assert.equal(privateNodes.length, 2)
    assert.equal(privateNodes[0]?.tag, 'biz')
    assert.equal(privateNodes[1]?.tag, 'bot')

    const groupNodes = nativeFlowAdditionalNodes('<jid-redacted@g.us>')
  ...
  it('carries the mixed native_flow child only when the interactive payload needs it', () => {
    const withMixed = nativeFlowAdditionalNodes('<jid-redacted@s.whatsapp.net>')[0]?.content
    ...
    const withoutMixed = nativeFlowAdditionalNodes('<jid-redacted@s.whatsapp.net>', false)[0]?.content
...
describe('MsgBuilder regression guards', () => {
  it('renders carousel cards in fallback text instead of an empty body', () => {
    const builder = MsgBuilder.to('<jid-redacted@s.whatsapp.net>').carousel([
      {
        title: 'Produk A',
  ...
  it('rejects AIRich combined with interactive components instead of dropping them silently', () => {
    assert.throws(
      () =>
        MsgBuilder.to('<jid-redacted@s.whatsapp.net>')
  ...
  it('omits the bot node for AIRich relays in group chats and keeps it in private chats', async () => {
    ...
    await MsgBuilder.to('<jid-redacted@g.us>').text('Isi laporan', { rich: true }).send(group.transport)
    ...
    await MsgBuilder.to('<jid-redacted@s.whatsapp.net>').text('Isi laporan', { rich: true }).send(direct.transport)
  ...
  it('logs a warning before falling back to text when relay throws', async () => {
    ...
    await MsgBuilder.to('<jid-redacted@s.whatsapp.net>')
      .text('Pilih aksi')
  ...
  it('refuses to build a blank plain-text payload', () => {
    assert.throws(() => MsgBuilder.to('<jid-redacted@s.whatsapp.net>').text('   ').build(), /must not be empty/)
  })
  ...
  it('tags AIRich payloads with the standard response type from the protobuf enum', () => {
    const built = MsgBuilder.to('<jid-redacted@s.whatsapp.net>').text('Ringkasan singkat.', { rich: true }).build()
    assert.equal(built.kind, 'airich')
