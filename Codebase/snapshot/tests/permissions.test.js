...
class PermissionCore {
  ...
  metadata = {
    ...
    participants: [
      ...
      { jid: '628120000009@s.whatsapp.net', role: 'member' },
    ],
  }
...
test('bot owner permission works outside groups and group owner permission remains separate', async () => {
  ...
  await core.emitMessage(message('owner-private', '628120000010@s.whatsapp.net', ownerJid, '!ownerprobe'))
  assert.equal(core.sent[0].text, 'owner command accepted')

  await core.emitMessage(message('member-private', '628120000010@s.whatsapp.net', '628120000003@s.whatsapp.net', '!ownerprobe'))
  assert.equal(core.sent[1].text, 'Maaf, command ini hanya tersedia untuk owner Allybot.')

  await core.emitMessage(message('creator-group', core.metadata.jid, '628120000001@s.whatsapp.net', '!groupownerprobe'))
  assert.equal(core.sent[2].text, 'group owner command accepted')

  await core.emitMessage(message('admin-group', core.metadata.jid, '628120000002@s.whatsapp.net', '!groupownerprobe'))
  assert.equal(core.sent[3].text, 'Maaf, command ini hanya dapat digunakan oleh pembuat grup.')

  await core.emitMessage(message('unknown-group', core.metadata.jid, '628120000002@s.whatsapp.net', '!unknownprobe'))
...
test('BOT_OWNER_JID accepts a phone JID and stays out of publicConfig', () => {
  const config = loadConfig({ NODE_ENV: 'test', BOT_OWNER_JID: '628120000009@s.whatsapp.net' })
  assert.equal(config.BOT_OWNER_JID, '628120000009@s.whatsapp.net')
