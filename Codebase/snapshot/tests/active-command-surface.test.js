...
test('bootstrap keeps Economy and removes retired category plugin registrations', () => {
  ...
  assert.match(entrypoint, /framework\.registerPlugin\(createKnowledgePlugin\(whatsapp\)\)/)
  assert.match(entrypoint, /framework\.registerPlugin\(createScenePlugin\(whatsapp\)\)/)
